...
system(Name, IniCbotCodeLink, Chatbots,System):-
    ...
    get_time(T),
    format_time(string(SysTime),'%FT%T%:z',T),
    System = [Name, IniCbotCodeLink, RevChatbots,[],0,SysTime].

%Predicado especial para reconstruir sistemas ya creados.
% system(Name,InitialChatbotCodeLink,Chatbots,Userlist,User,SysTime,System)
%Dominio: Name (string) X InitialChatbotCodeLink (int) X Chatbots (list of
%chatbots) X Userlist (list uf users) X User (string) X SysTime
%(string) X System (TDA system)
% Meta primaria: system/7
system(N,ICCL,CB,Userlist,User,SysTime,System):-
    System = [N,ICCL,CB,Userlist,User,SysTime].
...
% Metas secundarias: systemGetElements/7 , append/3 , chatbotsVerifier/2 , system/7
systemAddChatbot(SystemIni,Chatbot,SystemFin):-
    systemGetElements(SystemIni,E1,E2,ChatbotsIni,E4,E5,E6),
    append(ChatbotsIni,[Chatbot],Chatbots),
    chatbotsVerifier(Chatbots,ChatbotsFin),
    system(E1,E2,ChatbotsFin,E4,E5,E6,SystemFin).
...
%systemAddChatbot(System,Chatbot,System):-
%    systemGetElements(System,_,_,Chatbots,_,_,_),
...
% Metas secundarias: systemGetElements/7 , user/2 , append/3 , \+/1
% userInList/2 , system/7
systemAddUser(SystemIni,User,SystemFin):-
    systemGetElements(SystemIni,E1,E2,E3,UserlistIni,E5,E6),
    \+ userInList(User,UserlistIni),
    user(User,UserF),
    append(UserlistIni,[UserF],Userlist),
    system(E1,E2,E3,Userlist,E5,E6,SystemFin).
...
% Metas secundarias: systemGetElements/7 , !/0 , userInList/2 , system/7
systemLogin(System,User,SystemLog):-
    systemGetElements(System,E1,E2,E3,Userlist,0,SysTime),!,
    userInList(User,Userlist),
    system(E1,E2,E3,Userlist,User,SysTime,SystemLog).
...
% Metas secundarias: systemGetElements/7 , system/7
systemLogout(System,SystemOut):-
    systemGetElements(System,E1,_,E3,Userlist,_,SysTime),
    system(E1,0,E3,Userlist,0,SysTime,SystemOut).       %Un 0 en la última posición de system marca que no hay sesión iniciada
...
% Metas secundarias: \+/1 , systemGetElements/7
...
%    systemGetElements(System,_,_,_,_,User,_),
%    \+ string(User).

%Caso: mensaje NO es una opción válida.
% Metas secundarias: systemGetElements/7 , userInList/3 ,
% getChatbotFromList/3 chatbotGetElements/6 , getFlowFromList/3 , \+/2 ,
% get_time/1 , format_time/3 , msgInOpionList/4 , =/2 , append/3 ,
% user/3 , updateUserChatH/3 , system/7
systemTalkRec(System, Msg, SystemT):-
    systemGetElements(System,E1,CbotCodeLink,Chatbots,UserlistIni,User,SysTime),
    ...
    system(E1,CbotCodeLink,Chatbots,UserlistFin,User,SysTime,SystemT).

%Caso: mensaje SI es una opción válida
% Metas secundarias: systemGetElements/7 , userInList/3 ,
% getChatbotFromList/3 chatbotGetElements/6 , getFlowFromList/3 ,
% msgInOpionList/4 , get_time/1 , format_time/3 , =/2 append/3 , user/3 ,
% updateUserChatH/3 , updateChatbotFlowCode/4 , system/7
systemTalkRec(System,Msg,SystemT):-
    systemGetElements(System,E1,CbotCodeLinkIni,Chatbots,UserlistIni,User,SysTime),
    ...
    system(E1,NewCbotCodeLink,ChatbotsFin,UserlistFin,User,SysTime,SystemT).
...
% Metas secundarias: systemGetElements/7 , getUserChatH/3 , formatChatH/5
systemSynthesis(System, User, Str):-
    systemGetElements(System, _, _, Chatbots, Userlist, _, SysTime),
    getUserChatH(User,Userlist,ChatH),
    formatChatH(User, SysTime, Chatbots, ChatH,"", Str).
...
formatChatH(_, _, _,[],StrRes,StrRes).
formatChatH(User, SysTime,Chatbots,[[Time,CbotId,FlowId,Msg]|ChatH],StrAcum,StrRes):-
    ...
    chatToStr(User,Time,SysTime,Msg,CbotName,FlowMens,Oplist,ChatStr),
    string_concat(StrAcum,ChatStr,StrTemp),
    string_concat(StrTemp,"\n",StrAcumNew),
    formatChatH(User,SysTime,Chatbots,ChatH,StrAcumNew,StrRes).
...
%Predicado selector
% systemGetElements(System,Name,InitialChatbotCodeLink,Chatbots,Userlist,User,SysTime)
...
% X SysTime (string)
% Meta primaria: systemGetElemets/7
systemGetElements([E1,E2,E3,E4,E5,E6],E1,E2,E3,E4,E5,E6).
